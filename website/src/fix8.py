...
class Fix8:
    ...
    def generate_fixations(self):
        ...
        dispersion, ok = QInputDialog.getInt(
            self.ui, title, message, default_value, minimum_value, maximum_value
        )
        ...
        # generate fixations
        generated_fixations = np.array(
            mini_emtk.generate_fixations_left(self.aoi, dispersion)
        )

        self.eye_events = pd.DataFrame(
            generated_fixations, columns=["x_cord", "y_cord", "duration"]
        )
    ...
    def generate_fixations_skip(self):
        ...
        # generate fixations
        generated_fixations = np.array(
            mini_emtk.generate_fixations_left_skip(
                self.aoi, approximate_letter_width, lam, k_value
            )
        )
        self.eye_events = pd.DataFrame(
            generated_fixations, columns=["x_cord", "y_cord", "duration"]
    ...
    def generate_within_line_regression(self):
        ...
        probability, ok = QInputDialog.getInt(
            self.ui, title, message, default_value, minimum_value, maximum_value
        )
        ...
        # generate fixations
        generated_fixations = np.array(
            mini_emtk.within_line_regression(self.aoi, probability / 10)
        )
        self.eye_events = pd.DataFrame(
            generated_fixations, columns=["x_cord", "y_cord", "duration"]
        )
    ...
    def generate_between_line_regression(self):
        ...
        # generate fixations
        generated_fixations = np.array(
            mini_emtk.between_line_regression(self.aoi, probability / 10)
        )
        self.eye_events = pd.DataFrame(
            generated_fixations, columns=["x_cord", "y_cord", "duration"]
        )
    ...
    def generate_noise(self):
        ...
        self.metadata += (
            "generate,noise " + str(threshold) + "," + str(time.time()) + "\n"
        )
        self.save_state()

        self.eye_events["x_cord"] = self.eye_events["x_cord"] + np.random.normal(
            0, threshold, self.eye_events.shape[0]
        )
        self.eye_events["y_cord"] = self.eye_events["y_cord"] + np.random.normal(
            0, threshold, self.eye_events.shape[0]
        )
    ...
    def generate_slope(self):
        ...
        self.metadata += (
            "generate,slope " + str(threshold) + "," + str(time.time()) + "\n"
        )
        self.save_state()
    ...
    def generate_offset(self):
        ...
        self.metadata += (
            "generate,offset " + str(threshold) + "," + str(time.time()) + "\n"
        )
        self.save_state()
    ...
    def generate_shift(self):
        ...
        self.metadata += (
            "generate,shift " + str(threshold) + "," + str(time.time()) + "\n"
        )
        self.save_state()
        ...
        original_fixations = np.array(self.eye_events[["x_cord", "y_cord", "duration"]])
        fixations = np.array(
            mini_emtk.error_shift(threshold, line_Y, original_fixations)
        )
        self.eye_events["x_cord"] = fixations[:, 0]
        self.eye_events["y_cord"] = fixations[:, 1]
        self.eye_events["duration"] = fixations[:, 2]
    ...
    def calculate_fixation_report(self):
        ...
        qfd = QFileDialog()
        default_file_name = (
            self.trial_path.replace(".json", "") + "_fixation_report.csv"
        )
        file_name, _ = qfd.getSaveFileName(
            self.ui, "Save Fixations Report", default_file_name
        )
    ...
    def calculate_saccade_report(self):
        ...
        file_name, _ = qfd.getSaveFileName(
            self.ui, "Save Saccades Report", default_file_name
        )
    ...
    def calculate_aoi_report(self):
        ...
        radius, ok = QInputDialog.getInt(
            self.ui,
            "Radius",
            "Enter inclusion radius for hit test (pixels)",
            default_radius,
            minimum_value,
            maximum_value,
        )
        ...
        file_name, _ = qfd.getSaveFileName(
            self.ui, "Save AOI Report", default_file_name
        )
        ...
        original_fixations = np.array(self.eye_events[["x_cord", "y_cord", "duration"]])
        hit_test_data = mini_emtk.hit_test(
            original_fixations, self.trial_path, self.aoi, radius=radius
        )
    ...
    def calculate_aoi_metrics(self):
        ...
        file_name, _ = qfd.getSaveFileName(
            self.ui, "Save AOI Metrics Report", default_file_name
        )
        ...
        for index, row in self.aoi.iterrows():
            ...
            single_fixation_duration.append(
                mini_emtk.get_single_fixation_duration(hit_test_data, line, part)
            )
            first_fixation_duration.append(
                mini_emtk.get_first_fixation_duration(hit_test_data, line, part)
            )
            ...
            fixation_count.append(
                mini_emtk.get_fixation_count(hit_test_data, line, part)
            )
    ...
    def ascii_to_csv_converter(self):
        ...
        qfd = QFileDialog()
        ascii_file = qfd.getOpenFileName(
            self.ui, "Select ascii file", "", "ASCII Files (*.asc)"
        )[0]
        ...
        new_correction_file_name, _ = qfd.getSaveFileName(
            self.ui, "Save converted CSV file", default_file_name
        )
    ...
    def json_to_csv_converter(self):
        ...
        qfd = QFileDialog()
        json_file = qfd.getOpenFileName(
            self.ui, "Select Json file", "", "Json Files (*.json)"
        )[0]
    ...
    def csv_to_json_converter(self):
        ...
        qfd = QFileDialog()
        csv_file = qfd.getOpenFileName(
            self.ui, "Select CSV file", "", "CSV Files (*.csv)"
        )[0]
        ...
        new_correction_file_name, _ = qfd.getSaveFileName(
            self.ui, "Save converted json file", default_file_name
        )
    ...
    def eyelink_experiment_to_csv_converter(self):
        ...
        self.show_error_message("Warning", "Conversion may take a while")
        mini_emtk.read_EyeLink1000_experiment(
            ascii_file, save_folder, runtime_folder=runtime_folder
        )
    ...
    def outlier_duration_filter(self):
        ...
        message = (
            "Remove fixations with durations X standard deviations away from the mean"
        )
        ...
        self.eye_events = self.eye_events[
            ((self.eye_events["duration"] - mean) / std) < threshold
        ]
        ...
        if self.algorithm != "manual" and self.suggested_corrections is not None:
            ...
            self.suggested_corrections = self.suggested_corrections[
                self.suggested_corrections[:, 2] < mean + threshold * std
            ]
    ...
    def outside_screen_filter(self):
        self.metadata += (
            "filter,removed fixations outside screen," + str(time.time()) + "\n"
        )
    ...
    def lowpass_duration_filter(self):
        ...
        if self.algorithm != "manual" and self.suggested_corrections is not None:
            ...
            self.suggested_corrections = self.suggested_corrections[
                self.suggested_corrections[:, 2] > int(threshold)
            ]
    ...
    def merge_fixations(self):
        ...
        while index < len(new_fixations) - 1:
            ...
            if (
                new_fixations[index][2] <= duration_threshold
                or new_fixations[index + 1][2] <= duration_threshold
            ) and mini_emtk.distance(
                new_fixations[index], new_fixations[index + 1]
            ) <= dispersion_threshold:
        ...
        self.eye_events = pd.DataFrame(
            new_fixations, columns=["x_cord", "y_cord", "duration"]
        )
        self.eye_events["eye_event"] = "fixation"
    ...
    def outlier_metrics_filter(self):
        ...
        metrics_file = QFileDialog.getOpenFileName(
            self.ui, "Select metrics csv file", "", "CSV Files (*.csv)"
        )
        ...
        new_metrics_file = metrics_file[0].replace(
            ".csv", +"_" + abbreviated_metric + "_filtered.csv"
        )
    ...
    def run_correction(self):
        ...
        if "warp" not in self.algorithm:
            self.suggested_corrections[:, 0:2] = self.algorithm_function(
                fixation_XY, line_Y
            )
            ...
            # hybrid
            self.suggested_corrections[:, 0:2] = self.algorithm_function(
                fixation_XY, line_Y, word_XY, self.secondary_algorithm_function
            )
    ...
    def run_algorithm(
        ...
        algorithm_function,
        mode,
        secondery_algorithm_function=None,
    ):
        ...
        # write metadata
        self.metadata += (
            "selected, algorithm " + str(self.algorithm) + "," + str(time.time()) + "\n"
        )
    ...
    def manual_correction(self):
        ...
        # write metadata
        self.metadata += (
            "selected, " + str(self.algorithm) + "," + str(time.time()) + "\n"
        )
    ...
    def button_release_callback(self, event):
        ...
        if self.selected_fixation is not None:
            ...
            # move fixation
            self.eye_events.loc[self.selected_fixation, ["x_cord", "y_cord"]] = self.xy[
                self.selected_fixation
            ]
    ...
    def motion_notify_callback(self, event):
        ...
        if self.locked_x:
            self.xy[self.selected_fixation] = np.array(
                [self.eye_events.loc[self.selected_fixation, "x_cord"], y]
            )
        else:
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() in range(48, 58):
            self.metadata += (
                "key" + e.key() + ",assign fixation to line," + str(time.time()) + "\n"
            )
    ...
    def remove_fixation(self):
        if self.eye_events is not None and self.selected_fixation is not None:
            if self.selected_fixation < len(self.eye_events):
                ...
                if (
                    self.suggested_corrections is not None
                    and len(self.suggested_corrections) > 0
                ):
            ...
            if len(files) > 0:
                ...
                for file in files:
                    if (
                        ...
                        and file.endswith("_AOI.csv") == False
                        and file.endswith("_hit_test.csv") == False
                    ):
    ...
    def open_image(self):
        qfd = QFileDialog()
        self.image_file_path = qfd.getOpenFileName(
            self.ui, "Select Image", "", "Image Files (*.png *.jpg *.jpeg)"
        )[0]
    ...
    def open_trial(self):
        qfd = QFileDialog()
        self.trial_path = qfd.getOpenFileName(
            self.ui, "Select Trial file", "", "CSV or Json (*.csv *.json)"
        )[0]
    ...
    def update_trial_statistics(self):
        ...
        if self.eye_events is not None and "time_stamp" in self.eye_events.columns:
            duration = (
                self.eye_events.iloc[-1]["time_stamp"]
                - self.eye_events.iloc[0]["time_stamp"]
            )
            self.ui.statistics_table.setItem(
                0, 1, QTableWidgetItem(str(duration // 1000) + " sec")
            )
        else:
        ...
        if self.eye_events is not None:
            self.ui.statistics_table.setItem(
                1, 1, QTableWidgetItem(str(np.max(self.eye_events["duration"])))
            )
        else:
        ...
        if self.eye_events is not None:
            self.ui.statistics_table.setItem(
                2, 1, QTableWidgetItem(str(np.min(self.eye_events["duration"])))
            )
        else:
    ...
    def open_aoi(self):
        qfd = QFileDialog()
        aoi_file = qfd.getOpenFileName(self.ui, "Select AOI", "", "AOI Files (*.csv)")[
            0
        ]
    ...
    def draw_canvas(self, draw_all=False):
        ...
        if draw_all:
            x = np.array(
                self.eye_events[self.eye_events["eye_event"] == "fixation"]["x_cord"]
            )
            y = np.array(
                self.eye_events[self.eye_events["eye_event"] == "fixation"]["y_cord"]
            )
            duration = np.array(
                self.eye_events[self.eye_events["eye_event"] == "fixation"]["duration"]
            )
        else:
            x = np.array(
                self.eye_events[self.eye_events["eye_event"] == "fixation"][
                    "x_cord"
                ].iloc[0 : self.current_fixation + 1]
            )
            y = np.array(
                self.eye_events[self.eye_events["eye_event"] == "fixation"][
                    "y_cord"
        ...
        if self.ui.checkbox_show_fixations.isCheckable():
            if self.ui.checkbox_show_fixations.isChecked():
                ...
                self.ui.canvas.background = self.ui.canvas.copy_from_bbox(
                    self.ui.canvas.ax.bbox
                )
        ...
        if self.ui.checkbox_show_saccades.isCheckable():
            if self.ui.checkbox_show_saccades.isChecked():
                self.saccade_lines = self.ui.canvas.ax.plot(
                    x,
                    y,
                    alpha=self.saccade_opacity,
                    c=self.saccade_color,
                    linewidth=self.saccade_line_size,
                )
    ...
    def quick_draw_canvas(self, all_fixations=False):
        ...
        if self.ui.checkbox_show_suggestion.isChecked():
            if self.current_fixation < len(self.suggested_corrections):
                x = self.suggested_corrections[self.current_fixation][0]
                y = self.suggested_corrections[self.current_fixation][1]
                duration = self.eye_events[self.eye_events["eye_event"] == "fixation"][
                    "duration"
                ].iloc[self.current_fixation]
    ...
    def correct_all_fixations(self):
        ...
        if self.suggested_corrections is not None:
            self.save_state()
            self.eye_events.loc[
                self.eye_events["eye_event"] == "fixation", ["x_cord", "y_cord"]
            ] = self.suggested_corrections[:, 0:2]
    ...
    def confirm_suggestion(self):
        ...
        self.eye_events.loc[self.current_fixation, "y_cord"] = (
            self.suggested_corrections[self.current_fixation][1]
        )
    ...
    def save_metadata_file(self, new_correction_file_path):
        ...
        metadata_file_name = (
            new_correction_file_path.replace(".json", "") + "_metadata.csv"
        )
    ...
    def save_corrections_json(self):
        ...
        new_correction_file_name, _ = qfd.getSaveFileName(
            self.ui, "Save correction", default_file_name
        )
    ...
    def save_aoi_csv(self):
        qfd = QFileDialog()
        default_file_name = (
            self.trial_path.replace(".csv", "").replace(".json", "") + "_AOI.csv"
        )
        new_aoi_file_name, _ = qfd.getSaveFileName(
            self.ui, "Save AOI", default_file_name
        )
    ...
    def save_image(self):
        ...
        if self.trial_path is not None:
            default_file_name = (
                self.trial_path.replace(".csv", "").replace(".json", "") + "_FIX8.png"
            )
        else:
            default_file_name = "image_Fix8.png"

        new_image_file_name, _ = qfd.getSaveFileName(
            self.ui, "Save Image", default_file_name
        )
        ...
        if (
            ".png" not in new_image_file_name
        ...
        if ".svg" in new_image_file_name:
            self.ui.canvas.ax.figure.savefig(
                new_image_file_name,
                dpi=300,
                format="svg",
                transparent=True,
            ...
            self.ui.canvas.ax.figure.savefig(
                new_image_file_name, dpi=300, transparent=True, bbox_inches="tight"
            )
